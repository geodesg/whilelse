{map,each,find,concat,filter,join,fold1,elem-index,any} = prelude
...
j.callm = (obj-name, prop-name, ...args) -> j.call j.prop(obj-name, prop-name), ...args
...
j.throw = (expr) -> { type: 'ThrowStatement', argument: expr }
j.obj = (obj) ->
  g-props = []
...
j.assign-this = (prop-name, expr) -> { type: 'AssignmentExpression', operator: '=', left: j.memb(j.this!, j.id prop-name), right: expr }
j.assign-prop = (a, b, expr) -> { type: 'AssignmentExpression', operator: '=', left: j.prop(a, b), right: expr }
...
j.new = (callee, ...args) -> { type: 'NewExpression', callee: callee, arguments: args }
j.this = -> { type: 'ThisExpression' }
j.break = -> { type: 'BreakStatement' }
j.continue = -> { type: 'ContinueStatement' }

j.if = (test, consequent, alternate = null) -> { type: 'IfStatement', test, consequent, alternate }
j.not = (argument) -> { type: 'UnaryExpression', operator: '!', argument, prefix: true }
